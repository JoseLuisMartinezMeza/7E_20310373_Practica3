...
class Acomodo extends JPanel{
	...
	public Acomodo(){
		...
		reglamento.setForeground(Color.BLUE);				//Color de letras
...
//||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||    
    ...
    protected void paintComponent(Graphics g) {
    	try {
            backgroundImage = ImageIO.read(new File("P17.jpg"));  // Imagen a cambiar
        } catch (Exception e) {
            e.printStackTrace();
        }
